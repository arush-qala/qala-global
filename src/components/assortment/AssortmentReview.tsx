...
const ProductCard = ({ 
  ...
  return (
    <Reorder.Item
      value={product}
      id={product.id}
      className="flex-shrink-0 w-72 h-full cursor-grab active:cursor-grabbing group"
    >
...
export const AssortmentReview = ({ isOpen, onClose }: AssortmentReviewProps) => {
  const { products, reorderProducts, removeProduct } = useAssortment();
      ...
      {isOpen && (
        ...
        >
          ...
          <div 
            ...
            className="h-[calc(100%-88px)] overflow-x-auto overflow-y-hidden px-8 py-6"
          >
            {products.length > 0 ? (
              <Reorder.Group
                axis="x"
